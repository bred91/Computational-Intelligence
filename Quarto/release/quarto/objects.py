...
class Quarto(object):
    ...
    def run2(self, verbosity = False) -> int:
        '''
        A method added by me to run the game that collects the time taken to choose a piece and place it
        and removes (put it as optional) the print statements
        '''
        winner = -1
        max_c = 0
        max_p = 0
        while winner < 0 and not self.check_finished():
            ...
            piece_ok = False
            chose_e = time.time()
            if chose_e - chose_s > max_c:
                max_c = chose_e - chose_s
                board = self.get_board_status() 
                cont = 0
                for _ in range(4):
                    for _ in range(4):
                        if board[_][_] != -1:
                            cont += 1 
            # max_c = max(max_c, chose_e - chose_s)
            ...
            chose_e = time.time()
            max_p = max(max_p, chose_e - chose_s)
            winner = self.check_winner()
        if verbosity:
            self.print()
        #print(f"max c: {max_c}, max p: {max_p}")
        return winner, max_c, max_p, cont
